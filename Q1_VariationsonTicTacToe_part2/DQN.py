...
# from tf_agents.networks import EncodingNetwork
# from tf_agents.keras_layers import FlattenConcat
...
class IMAgent(DqnAgent):
    ...
    def _build_q_net(self):
        # Define the preprocessing layers for each part of the observation
        preprocessing_layers = {
            'board': tf.keras.layers.Flatten(),
            'energy': tf.keras.layers.Dense(units=2)
        }
        preprocessing_combiner = tf.keras.layers.Concatenate(axis=-1)

        fc_layer_params = (100, 80, 60)
